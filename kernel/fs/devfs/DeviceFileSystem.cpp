...
int DeviceFileSystem::open(const char * filePath,
                           uint64_t     flags,
                           struct file *fp)
{
...
size_t DeviceFileSystem::read(
    const char *path, bool user, char *buf, int offset, int n)
{
  // LOG_DEBUG("dev read");
...
size_t DeviceFileSystem::clear(const char *filepath,
                               size_t      count,
                               size_t      offset,
                               size_t &    written)
{
...
size_t DeviceFileSystem::truncate(const char *filepath, size_t size)
{
  return 0;
}

int DeviceFileSystem::get_file(const char *filepath, struct file *fp)
{
  return 0;
}

size_t DeviceFileSystem::touch(const char *filepath)
{
  return 0;
}

size_t DeviceFileSystem::rm(const char *filepath)
{
  return 0;
}

int DeviceFileSystem::ls(const char *filepath,
                         char *      contents,
                         int         len,
                         bool        user)
{
