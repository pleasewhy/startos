                            ...
                            uint64_t cmd_len, const uint8_t *tx_buff, uint64_t tx_len)
{
    ...
    uint8_t *v_buf = (uint8_t*)memAllocator.alloc();
    uint64_t i;
    for(i = 0; i < cmd_len; i++)
        v_buf[i] = cmd_buff[i];
    for(i = 0; i < tx_len; i++)
                              ...
                              const void *tx_buff, uint64_t tx_len, spi_transfer_width_t spi_transfer_width)
{
    ...
    switch(spi_transfer_width)
    {
        case SPI_TRANS_SHORT:
            ...
            buf = (uint32_t*)memAllocator.alloc();
            for(i = 0; i < tx_len; i++)
                                   ...
                                   uint64_t cmd_len, uint8_t *rx_buff, uint64_t rx_len)
{
    ...
    {
        case SPI_TRANS_INT:
            write_cmd = (uint32_t*)memAllocator.alloc();
