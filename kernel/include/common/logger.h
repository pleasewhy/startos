...
static constexpr cstr PastLastSlash(cstr a, cstr b)
{
  return *a == '\0' ?
             b :
             *b == '/' ? PastLastSlash(a + 1, a + 1) : PastLastSlash(a + 1, b);
}
...
inline void
OutputLogHeader(const char *file, int line, const char *func, int level)
{
  const char *type;
  switch (level) {
  ...
  printf("%s [%s:%d:%s:pid=%d] - ", type, file, line, func, myTask()->pid);
}
